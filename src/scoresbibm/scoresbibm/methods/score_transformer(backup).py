...
def run_train_transformer_model(
    ...
    for j in range(total_number_steps):
        ...
        if (j % print_every) == 0:
            print(f"Iteration {j}")
            print(
                f"    Expectation: Loss [{l_train:.3f}], Base Loss [{l_train_base:.3f}], Regularisation [{l_train_reg:.3f}]")
            print(
                f"    Iteration:   Loss {loss}, Base Loss {base_loss}, Regularisation {reg_term} (Score Hess. {score_hess}, Target Hess. {target_hess})")
            if l_val is not None:
                print(
                    f"    Validation:  Loss [{l_val:.3f}], Base Loss [{val_base_loss:.3f}], Regularisation [{val_reg_term:.3f}] (Score Hess. [{val_score_hess:.3f}], Target Hess. [{val_target_hess:.3f}]), E.S.Counter {early_stopping_counter}")
